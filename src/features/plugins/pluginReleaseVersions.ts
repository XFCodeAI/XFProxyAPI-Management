...
export const getGitHubRepositorySlug = (repository: string): string => {
  ...
  const withoutHost = trimmed.replace(/^github\.com\//i, '').replace(/^\/+|\/+$/g, '');
...
export const fetchPluginReleaseVersions = async (
  ...
  const slug = getGitHubRepositorySlug(repository);
  if (!slug) throw new Error('Repository is not a GitHub repository');
  ...
  if (!Array.isArray(result.body)) throw new Error('GitHub releases response is not a list');
