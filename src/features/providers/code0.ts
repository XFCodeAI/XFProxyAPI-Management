...
export const resolveCode0BaseUrl = (value: string | undefined | null): string => {
  ...
  const matched = CODE0_BASE_URL_OPTIONS.find((option) =>
    [
      option.baseUrl,
...
export const isCode0OpenAIProvider = (config: OpenAIProviderConfig | undefined | null): boolean =>
  Boolean(config && matchesCode0OpenAIBaseUrl(config.baseUrl));

export const isCode0ClaudeProvider = (config: ProviderKeyConfig | undefined | null): boolean =>
  Boolean(config && matchesCode0AnthropicBaseUrl(config.baseUrl));
...
export const isCode0GeminiProvider = (config: GeminiKeyConfig | undefined | null): boolean =>
  Boolean(config && matchesCode0GeminiBaseUrl(config.baseUrl));
