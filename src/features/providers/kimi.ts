...
export const resolveKimiBaseUrl = (value: string | undefined | null): string => {
  ...
  const matched = KIMI_BASE_URL_OPTIONS.find((option) =>
    [option.baseUrl, option.openaiBaseUrl, option.anthropicBaseUrl].some(
      (candidate) => normalized === normalizeBaseUrl(candidate)
    )
  );
