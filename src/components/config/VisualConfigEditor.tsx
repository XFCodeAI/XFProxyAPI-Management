...
import { fieldErrorClass, inputClass } from '@/components/ui/formStyles';
import {
  IconCode,
  IconKey,
  IconNetwork,
...
export function VisualConfigEditor({
  ...
  const sectionJumpTargetRef = useRef<VisualSectionId | null>(null);
  const sectionJumpUnlockTimerRef = useRef<number | null>(null);
  ...
  const clearSectionJumpLock = useCallback(() => {
    sectionJumpTargetRef.current = null;
    if (sectionJumpUnlockTimerRef.current !== null) {
      clearTimeout(sectionJumpUnlockTimerRef.current);
      sectionJumpUnlockTimerRef.current = null;
    }
  }, []);

  const lockSectionJumpTarget = useCallback((sectionId: VisualSectionId) => {
    sectionJumpTargetRef.current = sectionId;
    if (sectionJumpUnlockTimerRef.current !== null) {
      clearTimeout(sectionJumpUnlockTimerRef.current);
    }
    sectionJumpUnlockTimerRef.current = window.setTimeout(() => {
      sectionJumpTargetRef.current = null;
      sectionJumpUnlockTimerRef.current = null;
    }, 1400);
  }, []);
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        const lockedSectionId = sectionJumpTargetRef.current;
        if (lockedSectionId) {
          const targetVisible = visibleEntries.some((entry) => entry.target.id === lockedSectionId);
          if (!targetVisible) return;
          clearSectionJumpLock();
  ...
  }, [clearSectionJumpLock, isCurrentLayer, mode, sections]);
      ...
      {mode === 'simple' ? (
        <div className={styles.simpleView}>
          ...
          <div className={styles.simpleForm}>
            <div className={styles.simpleLeftStack}>
              <div className={styles.simpleConnectionPanel}>
                <div className={styles.simpleFieldsGrid}>
                  {hostField}
                  {portField}
                </div>
                {proxyUrlField}
          ...
          {isMobile ? (
            <div className={styles.mobileSectionNav}>
              <div
                ...
                aria-label={t('config_management.visual.section_jump', {
                  defaultValue: 'Section jump',
                })}
              >
                {sections.map((section, index) => (
                  <button
                    ...
                    <SectionGrid>
                      ...
                      <FieldAnchor fieldId="rmPanelRepo">
                        <Input
                          ...
                          placeholder="https://github.com/XFCodeAI/XFProxyAPI-Management"
