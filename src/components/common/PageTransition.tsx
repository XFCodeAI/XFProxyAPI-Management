...
const IOS_SHADOW_VALUE = '-20px 0 36px color-mix(in srgb, var(--foreground) 20%, transparent)';
...
const waitForAnimationCompletion = (animation: AnimationPlaybackControlsWithThen) => {
  const finished = (animation as AnimationPlaybackControlsWithThen & { finished?: Promise<unknown> })
    .finished;
  if (finished && typeof finished.then === 'function') {
    return finished.catch(() => undefined);
  }
  return Promise.resolve(animation).catch(() => undefined);
};
...
const buildLocationSignature = (location: Location) =>
  `${location.pathname}${location.search}${location.hash}`;

const buildLayerKey = (location: Location) => `${location.key}:${buildLocationSignature(location)}`;
...
export function PageTransition({
  ...
  const currentLayerKey = currentLayer?.key ?? buildLayerKey(location);
  ...
  const currentLayerSignature = currentLayer
    ...
    : buildLocationSignature(location);
  ...
  useLayoutEffect(() => {
    const nextLayerKey = buildLayerKey(location);
    if (currentLayerSignature === buildLocationSignature(location)) return;
  ...
  useLayoutEffect(() => {
    ...
    if (reduceMotion) {
      // Accessibility: skip spatial motion entirely, fall back to a short crossfade.
      if (exitingLayerEl) {
      ...
      // Vertical: split timing - exit accelerates, enter settles slowly.
      if (exitingLayerEl) {
    ...
    if (!activeAnimations.length) {
      ...
      void Promise.all(
        activeAnimations.map((animation) => waitForAnimationCompletion(animation))
      ).then(() => {
        if (cancelled) return;
        completeTransition();
      });
  ...
  }, [isAnimating, transitionVersion, resolveScrollContainer]);
