...
import { isMap, isSeq, parse as parseYaml, parseDocument, type YAMLSeq } from 'yaml';
import type {
...
export function parsePluginStoreAuthRules(raw: unknown): PluginStoreAuthRule[] {
  if (!Array.isArray(raw)) return [];
  return raw
    .map((item, sourceIndex): PluginStoreAuthRule | null => {
      const record = asRecord(item);
      if (!record) return null;
      const rule: PluginStoreAuthRule = {
        id: `plugin-store-auth-${sourceIndex}`,
        sourceIndex,
      ...
      return rule.match.trim() || rule.type !== 'none' ? rule : null;
    })
    .filter((rule): rule is PluginStoreAuthRule => Boolean(rule));
}

function writePluginStoreAuthRulesToDoc(doc: YamlDocument, rules: PluginStoreAuthRule[]): void {
  const path: YamlPath = ['plugins', 'store-auth'];
  const existing = doc.getIn(path, true);
  const existingItems = isSeq(existing) ? existing.items : [];
  const activeRules = rules.filter((rule) => rule.match.trim());
  if (activeRules.length === 0) {
    if (docHas(doc, path)) doc.deleteIn(path);
    return;
  }

  const nextSequence = doc.createNode(activeRules.map(() => ({}))) as YAMLSeq<unknown>;
  activeRules.forEach((rule, index) => {
    const existingNode =
      rule.sourceIndex !== undefined ? existingItems[rule.sourceIndex] : undefined;
    if (isMap(existingNode)) nextSequence.items[index] = existingNode;
  });
  doc.setIn(path, nextSequence);

  activeRules.forEach((rule, index) => {
    const rulePath: YamlPath = [...path, index];
    PLUGIN_STORE_AUTH_KEYS.forEach((key) => doc.deleteIn([...rulePath, key]));
    doc.setIn([...rulePath, 'match'], rule.match.trim());
    doc.setIn([...rulePath, 'type'], rule.type);
    if (rule.applyTo.length > 0) doc.setIn([...rulePath, 'apply-to'], [...rule.applyTo]);
    if ((rule.type === 'bearer' || rule.type === 'github-token') && rule.tokenEnv.trim()) {
      doc.setIn([...rulePath, 'token-env'], rule.tokenEnv.trim());
    }
    if (rule.type === 'basic') {
      if (rule.usernameEnv.trim()) {
        doc.setIn([...rulePath, 'username-env'], rule.usernameEnv.trim());
      }
      if (rule.passwordEnv.trim()) {
        doc.setIn([...rulePath, 'password-env'], rule.passwordEnv.trim());
      }
    }
    if (rule.type === 'header') {
      if (rule.headerName.trim()) doc.setIn([...rulePath, 'header-name'], rule.headerName.trim());
      if (rule.headerValueEnv.trim()) {
        doc.setIn([...rulePath, 'header-value-env'], rule.headerValueEnv.trim());
      }
    }
    if (rule.allowInsecure) doc.setIn([...rulePath, 'allow-insecure'], true);
  });
}
  ...
  const applyVisualChangesToYaml = useCallback(
    (currentYaml: string): string => {
      try {
        ...
        if (dirtyFields.has('credentialGroups')) {
          setStringListInDoc(
            doc,
            ['credential-groups'],
        ...
        if (dirtyFields.has('apiKeysText') || dirtyFields.has('apiKeyGroups')) {
        ...
        if (pluginsDirty) {
          ...
          if (dirtyFields.has('pluginStoreAuth')) {
            writePluginStoreAuthRulesToDoc(doc, values.pluginStoreAuth);
          }
        ...
        const claudeHeaderDefaultsDirty =
