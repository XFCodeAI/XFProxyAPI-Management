  ...
  return (
    <div className={styles.container}>
      <div className={styles.pageHeader}>
        ...
        <p className={styles.pageDescription}>{t('system_info.about_title')}</p>
      </div>
      <div className={styles.content}>
            ...
            <div className={styles.infoTile}>
              <div className={styles.tileHeader}>
                ...
                <TooltipButton
                  ...
                  label={t('system_info.version_check_button')}
                >
        ...
        <Card>
          <div className={styles.sectionHeader}>
            <h2 className={styles.sectionTitle}>{t('system_info.models_title')}</h2>
            <Button
              variant="secondary"
              size="sm"
          ...
          {modelStatus && (
            <div className={getStatusBadgeClass(modelStatus.type)}>{modelStatus.message}</div>
          )}
          {modelsError && <div className={fieldErrorClass}>{modelsError}</div>}
          {modelsLoading ? (
            <div className={fieldHintClass}>{t('common.loading')}</div>
          ) : models.length === 0 ? (
            <div className={fieldHintClass}>{t('system_info.models_empty')}</div>
          ) : (
            <div className={itemListClass}>
              {groupedModels.map((group) => {
                const iconSrc = getIconForCategory(group.id);
                return (
                  <div key={group.id} className={itemRowClass}>
                    <div className={itemMetaClass}>
                      <div className={styles.groupTitle}>
                        {iconSrc && <img src={iconSrc} alt="" className={styles.groupIcon} />}
                        <span className={itemTitleClass}>{group.label}</span>
                      </div>
