    ...
    <div className={styles.container}>
      ...
      <section className={styles.section}>
        <div className={styles.sectionHeader}>
          <h2>{t('proxy_pools.pool_list', { defaultValue: '代理条目' })}</h2>
        </div>
        ...
        <div className={styles.table}>
          ...
          {loading ? (
            ...
            poolRows.map(({ pool, status }) => (
              <div className={styles.tableRow} key={pool.id}>
                ...
                <div className={styles.healthCell}>
                  <span
                    className={
                      !pool.enabled || status?.configError || (status?.checked && !status.available)
                  ...
                  <span>{proxyPoolRegion(status) || status?.ip || status?.checkError || '-'}</span>
                </div>
      ...
      <section className={styles.section}>
        <div className={styles.sectionHeader}>
          <h2>{t('proxy_pools.usage_title', { defaultValue: '配置代理引用' })}</h2>
        </div>
          ...
          {usageRows.length === 0 ? (
            <div className={styles.emptyState}>
              {t('proxy_pools.no_usages', {
                defaultValue: '当前没有全局或 AI 供应商配置直接引用代理；凭证绑定请看上方“绑定凭证”列。',
              })}
            </div>
          ) : (
            usageRows.map((usage) => (
