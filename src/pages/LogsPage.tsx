...
export function LogsPage() {
  ...
  useEffect(() => {
    ...
    const handleEscape = (event: KeyboardEvent) => {
      ...
      if (document.querySelector('[data-modal-overlay="true"]')) return;
      ...
      <div className={styles.content}>
        {activeTab === 'logs' && (
          ...
          >
            ...
            {error && <div className={fieldErrorClass}>{error}</div>}
            ...
            <div className={styles.filters}>
              ...
              <ToggleSwitch
                ...
                label={
                  <TooltipElement
                    label={t('logs.show_raw_logs_hint', {
            ...
            {loading ? (
              <div className={fieldHintClass}>{t('logs.loading')}</div>
        ...
        {activeTab === 'errors' && (
          ...
          >
            <div className="flex flex-col gap-6">
              <div className={fieldHintClass}>{t('logs.error_logs_description')}</div>

              {isHomeRuntime && (
                <div className={getStatusBadgeClass('warning')}>
                  {t('logs.error_logs_home_unavailable')}
                </div>
              )}
              ...
              {errorLogsError && <div className={fieldErrorClass}>{errorLogsError}</div>}

              <div className={styles.errorPanel}>
                {loadingErrors ? (
                  <div className={fieldHintClass}>{t('common.loading')}</div>
                ) : errorLogs.length === 0 ? (
                  <div className={fieldHintClass}>{t('logs.error_logs_empty')}</div>
                ) : (
                  <div className={itemListClass}>
                    {errorLogs.map((item) => (
                      <div key={item.name} className={itemRowClass}>
                        <div className={cn(itemMetaClass, styles.errorItemMeta)}>
                          <div className={cn(itemTitleClass, styles.errorItemTitle)}>
                            {item.name}
                          </div>
                          <div className={itemSubtitleClass}>
                            {item.size ? `${(item.size / 1024).toFixed(1)} KB` : ''}{' '}
                            {item.modified ? formatUnixTimestamp(item.modified) : ''}
                          </div>
                        </div>
                        <div className={cn(itemActionsClass, styles.errorItemActions)}>
                          <Button
                            variant="secondary"
                            size="sm"
                            onClick={() => {
      ...
      >
        <div className={styles.errorLogViewer}>
          ...
          {selectedErrorLogLoading ? (
            ...
            <div className={fieldHintClass}>{t('logs.error_log_empty_content')}</div>
          ) : null}
        </div>
      </Modal>
